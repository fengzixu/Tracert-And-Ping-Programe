...
//const int ICMP_ECHO_MSG = 8;		//定义ICMP报文类型代码
...
//	char IcmpSendBuff[ICMP_DATA_SIZE+sizeof(ICMP_HEADER)] = {0};		//定义发送缓冲区
...
//
//	ptr_icmpheader->type = ICMP_ECHO_MSG;		//设定报文类型
//	ptr_icmpheader->code = 0;
//	ptr_icmpheader->id = (USHORT)GetCurrentProcessId();
//	memset(IcmpSendBuff+sizeof(ICMP_HEADER),'E',ICMP_DATA_SIZE);
//
//	USHORT usSeqNo = 0;
//	int TTL = 1;
//	BOOL QuitFlag = FALSE;
...
//		ptr_icmpheader->cksum = htons(checkSum(IcmpSendBuff,sizeof(ICMP_HEADER)+ICMP_DATA_SIZE));
//
//
//		DecodeResult.usSeqNo = ((ICMP_HEADER*)IcmpSendBuff)->seq;
//		DecodeResult.dwRoundTripTime = GetTickCount();
//		//发送数据报
//		sendto(sockRaw,IcmpSendBuff,sizeof(ICMP_HEADER)+32,0,(sockaddr*)&sendinfo,sizeof(sendinfo));
